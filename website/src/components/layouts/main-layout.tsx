...
export function MainLayout({ children, paperData, currentSectionId = "" }: MainLayoutProps) {
    ...
    <SelectionProvider>
      <div className="h-screen w-full bg-background">
        <ResizablePanelGroup direction="horizontal" className="h-full">
          ...
          {!isSidebarCollapsed && (
            <>
              <ResizablePanel defaultSize={20} minSize={15} maxSize={35} className="border-r">
                <PaperNavigation
          ...
          {/* Main Content Area */}
          <ResizablePanel className="h-screen overflow-y-hidden" defaultSize={isSidebarCollapsed ? 75 : 55} minSize={30}>
            <main className="overflow-y-auto h-screen">
              <PaperHeader paperTitle={paperData.paper.title} onToggleSidebar={toggleSidebar} />
        ...
        <PaperNavigationFooter currentSectionIndex={currentSectionIndex} totalSections={paperSections.length} onSectionNavigation={handleSectionNavigation} />
