...
export function TableBlockComponent({ block }: TableBlockProps) {
  return (
    <div className="my-6">
      {block.title && <h4 className="text-lg font-medium mb-2 text-foreground">{block.title}</h4>}
            ...
            {block.rows.map((row, rowIndex) => (
              <tr key={rowIndex} className={rowIndex % 2 === 0 ? "bg-background" : "bg-muted/50"}>
                {row.map((cell, cellIndex) => (
                  <td key={cellIndex} className="border border-border px-4 py-2 text-foreground">
                    {cell}
                  </td>
                ))}
              </tr>
